 ...
 * 输出: assets/resources/art/*.png（15 张）
 *
 * 美学约束：留白为主、墨色为骨、一色点睛。
...
const OUT_DIR = path.join(__dirname, '..', '..', 'assets', 'resources', 'art');
...
/** bg_paper 720x1280：整屏宣纸底，四角极淡墨晕，不透明 */
function genBgPaper() {
  const cv = paperBase(720, 1280, 1001);
  // 四角淡墨晕（很克制）
  const corners = [
    [40, 60, 300, 11],
    [690, 90, 260, 12],
  ...
  grain(cv, 1002, 0.02, 0.08);
  save(cv, 'bg_paper.png');
}

/** bg_battle 720x1280：宣纸底 + 下半部层叠远山 + 留白雾气，不透明 */
function genBgBattle() {
  const cv = paperBase(720, 1280, 2001);

  // 极远一抹山影（几乎看不见，给上半部一点生气）
  mountainLayer(cv, {
    yBase: 700, amp: 90, scale: 0.003, alpha: 0.05, fade: 150,
    seed: 21, ridge: false,
  });

  // 飞鸟两笔（人字点景）
  for (const [bx, by, s, sd] of [[500, 330, 13, 31], [548, 306, 9, 32]]) {
  ...
  // 远山（最淡）
  mountainLayer(cv, {
    yBase: 850, amp: 150, scale: 0.0032, alpha: 0.1, fade: 190,
    seed: 22, ridgeWidth: 1.8,
  });
  fogBand(cv, 800, 920, PAPER, 0.45, 23);

  // 中山
  mountainLayer(cv, {
    yBase: 1000, amp: 190, scale: 0.0042, alpha: 0.18, fade: 230,
    seed: 24, ridgeWidth: 2.2,
  });
  fogBand(cv, 950, 1060, PAPER, 0.5, 25);

  // 近山（最浓，压在底部）
  mountainLayer(cv, {
    yBase: 1180, amp: 260, scale: 0.005, alpha: 0.34, fade: 320,
    seed: 26, ridgeWidth: 3, yBottom: 1280,
  });

  // 山脚淡墨 grounding
  vwash(cv, 1150, 1280, INK, 0.0, 0.06);
